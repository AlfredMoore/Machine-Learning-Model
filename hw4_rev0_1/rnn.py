...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        V = b_vocab.shape
        # print("N",N,"T",T,"W",W,"H",H)
        # N,D -> N,H
        out_init_fc, cache_init_fc = fc_forward(features,W_proj, b_proj)
        # N,T-1 -> N,T-1,W
        out_word_embed, cache_word_embed = word_embedding_forward(captions_in,W_embed)
        # print(out_word_embed.shape)
        # T-1,N,W -> T-1,N,H
        out_rnn, cache_rnn = rnn_forward(np.swapaxes(out_word_embed,0,1),out_init_fc,Wx, Wh, b)
        # print(out_rnn.shape)
        # N,T-1,H -> N,T-1,V
        out_temp_fc, cache_temp_fc = temporal_fc_forward(np.swapaxes(out_rnn,0,1), W_vocab, b_vocab)
        # print(out_temp_fc.shape)
        
        loss, dx = temporal_softmax_loss(out_temp_fc,captions_out,mask)
        # print(dx.shape)
        # N,T,V -> N,T,H
        gradx_temp_fc, gradw_temp_fc, gradb_temp_fc = temporal_fc_backward(dx,cache_temp_fc)
        # print(gradx_temp_fc.shape)
        grads['W_vocab'], grads['b_vocab'] = gradw_temp_fc, gradb_temp_fc
        # T,N,H -> T,N,W
        gradx_rnn, gradh0_rnn, gradWx_rnn, gradWh_rnn, gradb_rnn = rnn_backward(np.swapaxes(gradx_temp_fc,0,1),cache_rnn)
        # print(gradx_rnn.shape)
        grads['Wx'], grads['Wh'], grads['b'] = gradWx_rnn, gradWh_rnn, gradb_rnn
        # N,T,W -> N,T
        gradw_embed = word_embedding_backward(np.swapaxes(gradx_rnn,0,1), cache_word_embed)
        # print(gradw_embed.shape)
        grads['W_embed'] = gradw_embed

        gradx_proj, gradw_proj, gradb_proj = fc_backward(gradh0_rnn,cache_init_fc)
        grads['W_proj'], grads['b_proj'] = gradw_proj, gradb_proj
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
          ...
          # print(next_h.shape)
          v = next_h @ W_vocab + b_vocab
          # print(v.shape)
          # print(captions.shape)
          next_word = np.argmax(v,axis=1)
          # print(next_word)
